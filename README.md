...
**4 · Assets — collect before scripting.** The approved metaphors are walked into a list of concrete visual needs — hero objects, characters, props, environments — and each need is resolved against the asset sources (§8f) *before any shot is written*. The decision is intelligent and honest: a matching SVG is vendored; a need nothing matches gets a **declared fallback** — typography (often stronger than a mediocre illustration), an icon-set glyph, art-direction primitives, or a `bespoke` escalation flag for the human. The output, `design/assets.json`, maps every need to its resolution with a one-line justification, and the human approves the availability picture at the gate. This stage exists because a shooting script written against assets that turn out not to exist is fiction — availability constrains the script, never surprises it.
